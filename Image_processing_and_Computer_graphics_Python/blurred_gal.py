from padding_new_matrix_127 import padder
import numpy as np
...
def kernel_action(matrix, o_row, o_col):#grabbing information from padded image
  ...
  for i in range(o_row):#length of row size of original image
    for j in range(o_col):#length of column size of original image
      kernel = np.array([[1,2,1],[2,4,2],[1,2,1]])#create a kernal matrix of ones
  ...
  return temp
matrix, o_row, o_col = padder("new2_image.png") #call in the padded image
...
img.save('new_image_gal_127.png')
